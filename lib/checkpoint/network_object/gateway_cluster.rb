module Checkpoint::NetworkObject
  ...
  class GatewayCluster
    ...
    def initialize(object_hash)
      ...
      needed_keys = [:class_name, :type, :interfaces, :comments, :ipaddr, :name, :cluster_members, :firewall]
      ...
      @cluster_members = object_hash[:cluster_members].map do |member|
        Checkpoint::NetworkObject::ClusterMember.new(member)
      end
    end
    ...
    def match(match_obj)
      ...
      if match_obj.type == :ip
        ...
        @cluster_members.each do |member|
          return true if member.match(match_obj)
        end
    ...
    def to_string
      str = "GatewayCluster: #{@name} (#{@my_ip_obj.to_s}) "
      str += " Interfaces: "
